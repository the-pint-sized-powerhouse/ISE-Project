...
def codepoint_to_url(codepoint, style):
    ...
    if style == "twemoji":
        ...
        patched = re.sub(
            ...
            lambda m: m.group(1),
            base.replace(" ", "-")
            .replace("fe0f-20e3", "20e3")
            .replace("1f441-fe0f-200d-1f5e8-fe0f", "1f441-200d-1f5e8"),
        )
        response = requests.get(
            "https://github.com/twitter/twemoji/raw/gh-pages/v/latest"
        )
        version = response.text if response.ok else None
        if version:
            return (
                "https://github.com/twitter/twemoji/raw/gh-pages/v/%s/72x72/%s.png"
                % (version, patched)
            )
        else:
            return (
                "https://github.com/twitter/twemoji/raw/master/assets/72x72/%s.png"
                % patched
            )
    elif style == "noto":
        return (
            "https://github.com/googlefonts/noto-emoji/raw/master/png/128/emoji_u%s.png"
            % base.replace(" ", "_")
        )
    elif style == "blobmoji":
        return (
            "https://github.com/C1710/blobmoji/raw/master/png/128/emoji_u%s.png"
            % base.replace(" ", "_")
        )
