{
  ...
  programs.spicetify = {
    ...
    theme = with config.lib.stylix.colors; {
      ...
      src = pkgs.runCommand "spicetify-stylix-theme" {} ''
                        mkdir -p $out
                ...
                card = ${base01-hex}
                shadow = ${base00-hex}
                selected-row = ${base02-hex}
                button = ${base0D-hex}
                button-active = ${base0C-hex}
                button-disabled = ${base03-hex}
                tab-active = ${base0D-hex}
                notification = ${base0B-hex}
                notification-error = ${base08-hex}
                misc = ${base03-hex}
                EOF

                        # user.css (OPTIONAL but common)
        ...
        --display-spicetify-banner-ascii: block;
        --display-music-banner-ascii: none;
        --border-radius: 0px;
        --border-width: 1px;
        --border-style: solid;
        ...
        *:not([style*="lyric" i] *, [class*="lyric" i], .main-entityHeader-title) {
        ...
        div[class*="minBlockSize_56px-padding_8px"] {
        ...
        --background-highlight: var(--spice-main-elevated);
        ...
        --background-tinted-base: rgba(var(--spice-rgb-highlight), 0.25);
        --background-tinted-highlight: rgba(var(--spice-rgb-highlight), 0.5);
        --background-tinted-press: rgba(var(--spice-rgb-highlight), 1);
        }
        ...
        [class*="listRow-isInteractive"]:hover,
        [class*="listRow-isInteractive"]:hover::after {
        background-color: rgba(var(--spice-rgb-highlight), 0.5) !important;
        }
        ...
        select:hover,
        select:focus {
        border: 1px solid var(--essential-base) !important;
        }
        ...
        > div:not([class*="custom"])
        > div:nth-child(1),
        .Root__globalNav > div:nth-child(3) > div:nth-child(2),
        .main-topBar-topbarContentRight + div {
        height: 0;
        width: 0;
        }
        ...
        .main-globalNav-historyButtonsContainer svg[class*="icon"],
        ...
        .main-globalNav-searchContainer
        form
        :is(#recent-searches-dropdown, #search-dropdown)
        > div {
        border-radius: var(--border-radius);
        box-shadow: none;
        margin: 24.5px 0;
        ...
        [class*="ChipInnerComponent-sm"] {
        border-radius: var(--border-radius);
        }
        .main-actionBar-ActionBar
        button:is([class*="button-primary"], [data-encore-id="buttonPrimary"]),
        .main-actionBar-ActionBar
        button:is(
        ...
        .main-actionBar-ActionBarRow > :first-child:not(:only-child) {
